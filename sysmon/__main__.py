...
defaults (CLI wins), and will hand the resulting :class:`~sysmon.config.settings.Settings` to
...
from sysmon.config.settings import load
...
def main(argv: list[str] | None = None) -> int:
    ...
    load(argv)
    print("sysmon: configuration parsed; engine not runnable yet.", file=sys.stderr)
    return 1
