...
PSYSMON is a **dependency-aware network monitoring daemon** — a modern Python rewrite of the 1998
C `sysmon`. It periodically checks hosts and services (ping, TCP, UDP, SMTP, POP3, DNS, HTTP/HTTPS),
...
The original `sysmon` (Jim Trocki, 1998) was a single-threaded C daemon driven by a positional
config file and a cleartext TCP control port. PSYSMON keeps the observable behavior — the check
types, the status codes, the dependency model, the status page — but rebuilds the internals:
