...
Every stanza starts with the **host** (field 1 — a hostname *or* an IP) and the **type** keyword
(field 2). The remaining positional fields depend on the type. `contact` is the notification
address; an absent contact means **syslog only, no page**.
